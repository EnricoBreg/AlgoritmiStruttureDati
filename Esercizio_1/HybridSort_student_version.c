...
void insertionSort(int A[], int low, int high){
    for(int j = low+1; j <= high ; j++ ){
        int key = A[j]; // chiave di confronto 
        int  i = j-1;
        while( i >= low && A[i] > key ){
            A[i+1] = A[i];
...
void merge(int *A, int low, int mid, int high) {
    ...
    int *left, *right;
    n1 = mid-low+1; // dimensione di left
    n2 = high-mid;   // dimensione di right
    ...
    /* Deallocazione della memoria riservata agli array di appoggio */
    free(left);
    free(right);
    return;
}
...
int main(int argc, char *argv[]) {
    ...
    clock_t timeHS = 0;         // HybridSortsortArray
